...
CARD_WIDTH = 567   # 15cm at 96 DPI
CARD_HEIGHT = 378  # 10cm at 96 DPI
...
class ProjectCardsAutomation(Automation):
    ...
    def run(self, ctx: AutomationContext) -> dict[str, Any]:
        ...
        output_folder = _resolve_output_folder(ctx)
        ...
        for note in sorted(vault_path.rglob("⌬*.md")):
            ...
            title = note.stem.lstrip("⌬").strip()
            image_uri = _image_to_data_uri(image_path)

            html = template.render(image_uri=image_uri, title=title)
...
def _resolve_output_folder(ctx: AutomationContext) -> Path:
    raw = ctx.config.settings.get("project_cards_output_folder", "output/project_cards")
    path = Path(str(raw)).expanduser()
    if not path.is_absolute():
        path = ctx.config.project_root / path
    return path
